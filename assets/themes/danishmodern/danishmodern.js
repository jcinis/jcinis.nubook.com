...
_.extend(Nubook, {
    onResize: function(complete){
        var minHeight = 683;
        var newHeight = $('#header').outerHeight() + $('#footer').outerHeight() + minHeight > $(window).height() ? minHeight : $(window).height() - $('#header').outerHeight() - $('#footer').outerHeight();
...
fontload.loadFont('etica-light',function(css) { fontload.writeStyle(css); });
...
var Neo = Nubook.Views.Theme.extend({
    ...
    render: function(){ // from neo
        ...
        if(!this.model.get('logo')) {
            ...
            if(!this.model.get('slogan')) {
                logoEl.addClass('no-slogan');
            } else {
                logoEl.removeClass('no-slogan');
            }
        } else {
            var logoEl = $('#logo');
...
var NeoMenu = Backbone.View.extend({
    ...
    render: function(){
        ...
        this.el.find('li:first').addClass('first-child');
        return this;
    }
});
