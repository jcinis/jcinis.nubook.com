...
_.extend(Nubook, {
    onResize: function(complete){
        ...
        if($(window).height() > $('#content').height()) {
            ...
            $('#stage').css('height',$('content').height()+'px');
        }
        
        $('#content').css({
            'left': Math.floor(($("#stage").width()-$('#content').width()) / 2)+'px',
            'top': Math.floor(($("#stage").height()-$('#content').height()) / 2)+'px'
        });
...
fontload.loadFont('etica',function(css) { fontload.writeStyle(css); Nubook.onResize(function(){ $('#stage').animate({opacity:1},'slow'); }); });
...
var GrayCard = Nubook.Views.Theme.extend({
    ...
    initialize: function(){
        ...
        this.menu = new GrayCardMenu({collection:Nubook.data.categories});
        this.pagination = new GrayCardPagination();
        this.sitemap_view = new GrayCardSiteMap();
        this.render();
    },
    ...
    page: function(page){
        ...
        page.view.render(function(view){
            ...
            // SET DATE
            $('#frame .date').text(isoformatToLocal(page.get('created')).format('F jS, Y'));
...
var GrayCardMenuItem = Backbone.View.extend({
...
var GrayCardMenu = Backbone.View.extend({
    ...
    render: function(){
        ...
        this.collection.each(function(item, i){
            if(!item.get('is_default') && item.get('published_ids').length > 0) {
                item.menuView = new GrayCardMenuItem({model:item});
...
var GrayCardPagination = Backbone.View.extend({
    ...
    render: function(page){
        ...
        if(Nubook.data.blogmode) {
            ...
            var category_id = category ? category.id : 'blog';
            if(!category){
...
var GrayCardSiteMap = Backbone.View.extend({
    ...
    render: function(complete){
        ...
        Nubook.data.categories.each(function(item,i){
            if(item.get('published_ids').length > 0) {
                item.sitemapView = new GrayCardSiteMapCategory({model:item});
...
var GrayCardSiteMapCategory = Backbone.View.extend({
    ...
    render: function(){
        ...
        _.each(this.model.get('published_ids'),function(id,i){
            ...
            if(page) {
                page.sitemapView = new GrayCardSiteMapPage({model:page});
...
var GrayCardSiteMapPage = Backbone.View.extend({
    tagName: 'li',
