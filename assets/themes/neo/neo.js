...
_.extend(Nubook, {
    onResize: function(complete){
        var newHeight = $('#header').outerHeight() + $('#footer').outerHeight() + 620 > $(window).height() ? 620 : $(window).height() - $('#header').outerHeight() - $('#footer').outerHeight();
        if(newHeight <= 620) {
        ...
        $('#frame').css({'top':($("#stage").height() - $('#frame').outerHeight()) / 2});
        $('.pagination-arrow').css('top', ($("#stage").height() / 2 - 35)+'px');
